...
%\label{chap9:fig-adaptativeSubdivision}
%\end{figure}
%
...
%\begin{figure}
%\centering
%\includegraphics[width=12cm]{chapter9/retiling.pdf}
...
By using the Hausdorff distance between two iterations, the process may be stopped when the required precision has been reached. A simple example is shown \fig{chap9:fig-cylinder} to illustrate the method. 
\begin{figure}[ht]
\centering
\includegraphics[width=13.5cm]{chapter9/exampleCylinder.pdf}
...
This approach has been applied to approximate more complex anatomical geometries with curved shell elements. Two examples are given with the Glisson's capsule, which is the membrane surrounding the liver (\fig{chap9:fig-liver}) and an aneurysm (\fig{chap9:fig-aneurysm}). In each case, the error is computed through a Hausdorff distance and expressed as a percentage of the diagonal of the object's bounding box. 
%
\begin{figure}[ht]
\centering
\includegraphics[width=13.5cm]{chapter9/resultsLiver.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=13.5cm]{chapter9/resultsAneurysm.pdf}
...
We perform several tests on the aneurysm model at different resolutions to measure computation times. The shells are resisting to a uniform pressure load and solved using a Conjugate Gradient (CG) iterative solver. The computation times are reported in \fig{chap9:fig-computation}. Implicit integration allows for large time steps ($40\,ms$) and the computation is real-time for 800 shell elements and a reasonable error criterion ($5\,\%$). When the computation time must be bounded (critical real-time applications), one can fix the number of CG iterations to, for instance, $100$ and remains real-time for $1\,000$ shell elements. However, in that case the accuracy of the results is not checked.
%
\begin{figure}[ht]
\centering
\includegraphics[width=13.5cm]{chapter9/computation_times.pdf}
