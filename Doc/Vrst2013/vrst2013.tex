...
In the second step, we use our simulation to reproduce the displacements values measured experimentally in~\cite{Hollenstein2006,Nava2008}.
Since the Young's moduli of neither parenchyma ($E_p$) nor capsule ($E_c$ were specified, we obtained both as follows: first, 
performed a set of simulations without the capsule for different values of ($E_p$): the value that provided a good match 
with reported displacement after applying the negative pressure of 30\,kPa inside the tube was $E_p$=30\,kPa. 
The, we fixed the thickness of the capsule as $T$=93\,$\mu$m (reported as an average in the paper) and 
using $E_p$=30\,kPa, we performed repeatedly the simulation with the capsule using different values of the Young's modulus
of the capsule in order to minimize the displacement error. The minimal error (under 1\%) was obtained 
for Young's modulus $E_c$=3\,MPa which lies in the range of capsule elasticity reported by experimental measurements.
