...
mechanical influence of vessels on both global and local behaviour of the organ has been measured and modelled previously~\cite{Peterlik2012}, only local 
effects have been studied in the case of Glisson's capsule~\cite{Hollenstein2006,Ahn2010}. 

In this paper we introduce what we believe is the first real-time model of liver taking into account all three constituents, each being simulated 
employing a different finite element formulation.
Using the proposed model, we focus on the role of the Glisson's capsule: first, we show that the model is able to reproduce correctly
...
We can observere that the deformation without the capsule is indeed overestimated approximately by the factor of 2. This is in perfect agreement with the results reported in~\cite{Hollenstein2006}

Moreover, if we neglect the time-dependent behaviour we are able to reproduce
the measurements presented in~\cite{Hollenstein2006} with less than 1\% error.
Simulating the pressure of 3\,MPa and setting the elastic parameters to
$E_p$=30\,kPa for parenchyma and $E_c$=3\,MPa and $t$=93\,$\mu$m for capsule.
All the values lie within the measurements specified in the paper.
\TG{add figure?}
...
%\begin{figure*}
%  \centering
%    \subfloat[]{\includegraphics[width=.32\textwidth]{gravity2.png}}
%    \subfloat[]{\includegraphics[width=.32\textwidth]{registered.png}}
%    \subfloat[]{\includegraphics[width=.32\textwidth]{distanceMap.png}}
...
\TG{including vascularization; global deformation; stress the complexity of validation of global deformations}
