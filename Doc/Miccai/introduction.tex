...
In this paper we present an improved approach for predictive simulation of congenital heart defect (CHD) corrective surgeries using a physics based model of the vessel walls: their mechanical behavior is simulated using elastic thin shell elements. 
Our method is not limited to well-formed blood vessels and captures all low-level procedures necessary for aortoplasty. We use a relaxation process to arbitrarily join or attach blood vessels and patches without using springs that would add surplus energy to the physical system. By taking the overall tubular structure of blood vessels into consideration during generation of thin shell elements we achieve reasonable speed of the simulation.
