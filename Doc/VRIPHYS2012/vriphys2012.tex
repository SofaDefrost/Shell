...
For that we can reuse equation (\ref{bez-delta}) and apply the derivatives. For instance, for the in-plane displacement:
$$
\frac{\delta \epsilon_{xx}} {\delta \mathbf{x}_{2}} = \sum_{i=1}^{3} (\frac{\partial n_2}{\xi_i} + \frac{\partial n_6}{\xi_i} + \frac{\partial n_9}{\xi_i} + \frac{1}{3}\frac{\partial n_{10}}{\xi_i})\frac{\partial \xi_i}{\partial x}  
$$
...
\CD{or... $K =  \iiint_V \left[\begin{matrix} J_m & J_b \end{matrix} \right]^T  M \left[\begin{matrix} J_m & J_b \end{matrix} \right]$. Could we test that option easily in the code, or is it a nightmare ??}
