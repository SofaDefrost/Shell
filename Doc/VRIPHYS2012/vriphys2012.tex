...
\noindent
and variation of central point $\delta P_{10}$ \CD{ this formulation would be modified...} :
...
The strain tensor has additional contributions based on Kirchhoff-Love theory for thin plates:
\begin{equation}
\label{out-plane-strain}
\left[ \begin{array}[c]{c} \epsilon_{xx} \\ \epsilon_{yy}  \\ 2\epsilon_{yx} \end{array} \right] +=
\left[ \begin{array}[c]{c} -z \frac{\partial^2 u^z}{\partial x^2}  \vspace{1mm}\\ -z \frac{\partial^2 u^z}{\partial y^2}   \vspace{1mm}\\ - 2z \frac{\partial^2 u^z}{\partial xy}    \end{array} \right]
\end{equation}
%% comment
\CD{Here, if you want to keep a separate computation for bending and membrane (see the folllowing), use $=$ instead of $+=$ !!}
%%
where $z$ is the local coordinate along $z$-axis of the point on which the strain is measured.


As the term  $\frac{\partial \xi_i}{\partial l}$ provides a constant value, the second derivatives can be computed quite easily, for instance, for $\epsilon_{xx}$:
\begin{equation}
\frac{\partial^2 u^z}{\partial x^2}  = \sum_{i=1}^3\frac{\partial}{\partial \xi_i}\left(\sum_{j=1}^3 \frac{\partial T^z}{\partial \xi_j} \frac{\partial \xi_j}{\partial x}\right) \frac{\partial \xi_i}{\partial x} 
=  \sum_{i=1}^3  \sum_{j=1}^3 \frac{\partial^2 T^z}{\partial \xi_i\xi_j}   \frac{\partial \xi_j}{\partial x} \frac{\partial \xi_i}{\partial x}  
\end{equation}
...
As previously describes, the shell element's strain is a combination of two types of deformations, as shown in figure \ref{deformations}.
If we assume that the initial out-plane deflections are negligible and that the frame of the shell element nodes are aligned with local $z$ axis, we can distribute these two deformations into two groups of degrees of freedom at the nodes' level.

\FigureWI{deformations}
{Plate deformations:%
a) membrane deformations like stretching or shearing, due to in-plane displacements
b) undeformed;
c) bending deformation, due to out-plane displacements
\label{deformations}}
...
(Figure \ref{fig-membrane}).
    ...
    \theta_z$ and out of plane displacement $\delta\mathbf{x}_z$ (Figure \ref{fig-plate}).
...
To illustrate, let's take one value of the matrix $\left[ \mathbf{J}_m \right]$  (first line, fourth column), we first take the kinematic between $\delta T_x$ and the concerned degree of freedom $ \delta \mathbf{x}_{2x}$:
$$
   \frac{ \delta T_x}{ \delta \mathbf{x}_{2x}} = (n_2 + n_6 + n_7 + \frac{1}{3} n_{10})
$$
Then, we apply the spacial derivatives to compute the variation of $\epsilon_{xx}$ when 
$$
\delta \epsilon_{xx} += \underbrace{\sum_{i=1}^{3} (\frac{\partial n_2}{\xi_i} + \frac{\partial n_6}{\xi_i} + \frac{\partial n_9}{\xi_i} + \frac{1}{3}\frac{\partial n_{10}}{\xi_i})\frac{\partial \xi_i}{\partial x}}_{\left[ \mathbf{J}_m \right](1,4)}  {\delta \mathbf{x}_{2x}}
$$
...
\begin{align}
    \label{eq-Km}
    \mathbf{K}_m & = \iiint_V \mathbf{J}_m^T M \mathbf{J}_m \, \mathrm{d} V \\
    \label{eq-Kb}
    \mathbf{K}_b & = \iiint_V \mathbf{J}_b^T M \mathbf{J}_b \, \mathrm{d} V
\end{align}
%% comment
\CD{This is something that is maybe wrong in our formulation... as the bending adds a contribution on the deformation tensor, maybe we should not separate the contribution from membrane and bending as we do now ($K_m$ and $K_b$ are computed separately) }
%% comment
\CD{here, it would be... $\mathbf{K}_e =  \iiint_V \left[\begin{matrix} \mathbf{J}_m & \mathbf{J}_b \end{matrix} \right]^T  M \left[\begin{matrix} \mathbf{J}_m & \mathbf{J}_b \end{matrix} \right]$. Could we test that option easily in the code, or is it a nightmare ??}
%%
