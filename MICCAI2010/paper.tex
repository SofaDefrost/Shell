...
%To develop this kind of interactive systems, one faces the trade-off problem between accuracy of the model and the need for fast computations. 
...
% even if there are obvious advantages to make maximum use of optimised generic models. 
...
% , triangular elements with 18 degrees of freedom, => pas vraiment, on s'interesse seulement a x,y en translation dans le plan et la rotation autour de x et y pour le bending... 
...
%We propose to determine the shape (polynomial) functions used in our shell formulation to approximate the surface of anatomical structures with a surface composed of shell elements.   => pas vraiment... pour un maillage donne elles sont fixees pour un triangle par les normales de chaque sommet. On joue sur la densite d'elements pour bien matcher la surface plutot...
