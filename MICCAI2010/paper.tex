...
%A mechanically-driven method to mesh curved surfaces with shells}
\label{sec:mesh}
...
This approach has been applied to approximate more complex anatomical geometries with curved shell elements.
In each case the error is expressed as a percentage of the diagonal of the object's bounding box. 
\begin{figure}[h]
\vspace{-0.8cm}
\centering
...
When the computation time must be bounded (critical real-time applications), one can fix the number of CG iterations to, for instance, $100$ and remain real-time for 1000 shell elements. However, in that case, the accuracy of the results is not checked.
%
\begin{figure}
\vspace{-0.5cm}
\centering
...
\caption {Simulation of an angioplasty procedure. (a, c): A collapsed stent is inserted into the blood vessel simulated with our shell FEM formulation. The fatty deposits (in yellow) are modelled with tetrahedral FEM.  (b, d): The stent is crushing the fatty deposits which create a pressure onto the interior wall and widen the blood vessel.}
\label{fig-stent}
\end{figure}
%
\section{Conclusion}
In this paper, we propose a framework for real-time modelling of thin anatomical structures. The novelty of our method relies on the combination of a shell finite element formulation and a geometric surface reconstruction both based on the same polynomial interpolation function used to describe the surface of shells. We also show how contacts and interactions with the curved surfaces of shells can be handled using the same function. The efficiency of the proposed method is illustrated through shell-based reconstruction and real-time simulation of the deformations of various anatomical structures. We also present preliminary results on an application of our work through the simulation of an angioplasty procedure. 
%
