...
\title[Simulation à base de Repères]{Simulation en Lagragien Total Implicite à base de Repères}

\author[Julien Bosman]{Julien Bosman$^1$, Christian Duriez$^2$ \\ %
...
limitent leur utilisation notamment leur failbe précision. Le traitement des frontières est également difficile en raison de la déficience en
...
une orientation qui est estimée au grâce à un \emph{Shape Matching} généralisé.
\section{Simulation}
...
dans la configuration déformée. La fonction de forme $w_s(\cdot)$ peut être à support compact (du type \emph{fonction de Wendland}) ou non (fonction de Shepard
par ex).
...
qu'ils sont étudiés à une échelle suffisament petite, mais aussi suffisament grande pour négliger les espaces inter-moléculaires. La MMC permet de mettre en
...
contrainte de \emph{Piola-Kirchoff} dans la notation de \emph{Voigt}. La dérivée du tenseur de Green-Lagrange par rapport au degrés de liberté
des repères est donné par :
\begin{equation}
...
\begin{equation}
	\delta f = \int_{\Omega}{\mathbf{M^t}\mathbf{C}\mathbf{M}} \cdot \delta \mathbf{q} d\Omega
\end{equation}
on obtient alors, en version discrète :
\begin{equation}
	\delta f_i = \sum_{s}{\mathbf{M}^{i,s}\cdot \Delta \mathbf{S}^s}
\end{equation}
avec : 
\begin{equation}
