...
void TriangularBendingFEMForceField<DataTypes>::init()
{
    ...
    if (d_refineMesh.getValue())
    {
        ...
        if (_topologyTarget)
        {
            MechanicalState<sofa::defaulttype::Vec3Types>* mStateTarget = dynamic_cast<MechanicalState<sofa::defaulttype::Vec3Types>*> (_topologyTarget->getContext()->getMechanicalState());
            if (mStateTarget)
            {
...
{
    sofa::type::vector<TriangleInformation>& triangleInf = *(triangleInfo.beginEdit());

    if (l_topologyMapper.get() != nullptr) {

        shell::engine::JoinMeshPoints<DataTypes>* jmp = l_topologyMapper.get();
...
template<class DataTypes>
void TriangularBendingFEMForceField<DataTypes>::addKToMatrix(const sofa::core::MechanicalParams* mparams, const sofa::core::behavior::MultiMatrixAccessor* matrix)
{
...
template <class DataTypes>
void TriangularBendingFEMForceField<DataTypes>::draw(const sofa::core::visual::VisualParams* vparams)
{
    ...
    if (m_mapTopology){
        ...
        if (points.size() > 0)
            vparams->drawTool()->drawLines(points, 1.0f, sofa::type::RGBAColor(1.0, 0.0, 1.0, 1.0));
    }
}
...
{
    if (dynamic_cast<shell::objectmodel::MeshChangedEvent*>(event))
    {
